...
def run_training(yaml_name=None):
    ...
    for _ in trainer.model.runs:
        ...
        for epoch_idx in range(trainer.num_epochs + 1):
            # Training
            ...
            if loss > trainer.max_loss:
                ...
                break
            if epoch_idx == trainer.num_epochs:
                print('Reached limit on number of epochs')
                save_model = True
                break
        if save_model is True:
            trainer.save_estimator()
...
if __name__ == '__main__':
    ...
    run_training('run/gaze_train_test.yaml')
